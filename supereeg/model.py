...
class Model(object):
    ...
    def __init__(self, data=None, locs=None, template=None, gpu=False,
        ...
        if not (data is None):
            ...
            if isinstance(data, Model):
                ...
                self.denominator = torch.zeros_like(self.numerator, dtype=torch.float64)
        ...
        if not ((numerator is None) or (denominator is None)):
            ...
            if (self.numerator is None) or (self.denominator is None):
                ...
                self._set_numerator(torch.logaddexp(self.numerator.real, numerator.real),
                                    torch.logaddexp(self.numerator.imag, numerator.imag))
                self.denominator = torch.logaddexp(self.denominator, denominator)
    ...
    def set_locs(self, new_locs, force_include_bo_locs=False):
        ...
        if _empty(self.locs):
            ...
            if not _empty(new_locs):
                self.numerator = torch.log(self.zeros([new_locs.shape[0], new_locs.shape[0]], dtype=torch.complex128))
                self.denominator = torch.zeros_like(self.numerator, dtype=torch.float64)
                self.locs = new_locs
            ...
            if not force_include_bo_locs:
                ...
                self.n_locs = 0
                self.numerator = torch.tensor([], dtype=torch.complex128)
                self.denominator = torch.tensor([], dtype=torch.float64)
            return
    ...
    def update(self, data, inplace=True):
        ...
        m1._set_numerator(torch.logaddexp(m1.numerator.real, m2.numerator.real),
                          torch.logaddexp(m1.numerator.imag, m2.numerator.imag))
        ...
        m1.denominator = torch.logaddexp(m1.denominator, m2.denominator)
        m1.locs = locs
    ...
    def _set_numerator(self, n_real, n_imag):
        ...
        """
        self.numerator = torch.zeros_like(n_real, dtype=torch.complex128)
    ...
    def plot_data(self, savefile=None, show=True, **kwargs):
        ...
        if corr_mat.shape[0] < 2000:
            ax = sns.heatmap(corr_mat.numpy(), cbar_kws = {'label': 'correlation'}, **kwargs)
        else:
            if savefile == None:
...
def _mo2model(mo, locs, width=20):
    ...
    if locs.equals(mo.locs):
        return mo.numerator.detach().clone(), mo.denominator.detach().clone(), mo.n_subs
    else:
...
def _force_update(mo, bo, width=20):
    ...
    n = mo.numerator.detach().clone()
    n.real = torch.logaddexp(n.real, num_corrmat_x.real)
    n.imag = torch.logaddexp(n.imag, num_corrmat_x.imag)
    return _recover_model(n, torch.logaddexp(mo.denominator, denom_corrmat_x), z_transform=True)
